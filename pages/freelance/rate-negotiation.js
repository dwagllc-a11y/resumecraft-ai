...
const FIELDS = ['freelancerName', 'skill', 'currentRate', 'targetRate', 'yearsExp', 'situation']
...
export default function Page() {
  ...
  const handlePay = async () => {
    ...
    try {
      const res = await fetch('/api/checkout-freelance-rate-negotiation', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
  ...
  const handleGenerate = async () => {
    setLoading(true); setError('')
    try {
      const res = await fetch('/api/generate-freelance-rate-negotiation', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
      const data = await res.json()
  ...
  return (
    <>
      <Head><title>Rate Negotiation Scripts — CareerCraft AI</title><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;600;700;800&display=swap" rel="stylesheet" /></Head>
      <div style={S.page}>
        <nav style={S.nav}>
        ...
        <div style={S.wrap}>
          <div style={{ marginBottom: '28px' }}>
            <div style={{ fontSize: '11px', color: '#f59e0b', fontWeight: '700', letterSpacing: '0.1em', marginBottom: '8px' }}>FREELANCECRAFT AI</div>
            <h1 style={{ fontSize: '32px', fontWeight: '800', letterSpacing: '-0.03em', marginBottom: '8px' }}>Rate Negotiation Scripts</h1>
            <p style={{ color: '#8a8070', fontSize: '15px' }}>Charge what you are worth and get it. $16.99 one-time.</p>
          </div>
