...
export default function Page() {
  ...
  const handlePay = async () => {
    ...
    try {
      const res = await fetch('/api/checkout-freelance-client-email', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
  ...
  const handleGenerate = async () => {
    setLoading(true); setError('')
    try {
      const res = await fetch('/api/generate-freelance-client-email', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
      const data = await res.json()
  ...
  return (
    <>
      <Head><title>Client Email Templates — CareerCraft AI</title><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;600;700;800&display=swap" rel="stylesheet" /></Head>
      <div style={S.page}>
        <nav style={S.nav}>
        ...
        <div style={S.wrap}>
          <div style={{ marginBottom: '28px' }}>
            <div style={{ fontSize: '11px', color: '#fb923c', fontWeight: '700', letterSpacing: '0.1em', marginBottom: '8px' }}>FREELANCECRAFT AI</div>
            <h1 style={{ fontSize: '32px', fontWeight: '800', letterSpacing: '-0.03em', marginBottom: '8px' }}>Client Email Templates</h1>
            <p style={{ color: '#8a8070', fontSize: '15px' }}>5 scripts for every freelance situation. $8.99 one-time.</p>
          </div>
          ...
          {!paid ? (
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '24px', textAlign: 'center' }}>
              <div style={{ fontSize: '32px', fontWeight: '900', color: '#fb923c', marginBottom: '4px' }}>$8.99</div>
