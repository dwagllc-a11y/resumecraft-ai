...
const FIELDS = ['freelancerName', 'skill', 'clientName', 'projectDesc', 'rate', 'paymentTerms', 'revisions']
const S = { page: { minHeight: '100vh', background: '#0d0f1a', fontFamily: "'DM Sans', sans-serif", color: '#f0ece0' }, nav: { padding: '0 32px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.07)', background: 'rgba(0,0,0,0.3)' }, wrap: { maxWidth: '700px', margin: '0 auto', padding: '48px 20px' }, label: { display: 'block', fontSize: '11px', fontWeight: '700', letterSpacing: '0.08em', color: '#8a8070', marginBottom: '8px' }, input: { width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '12px 16px', color: '#f0ece0', fontSize: '14px', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }, btn: { background: '#34d399', color: '#0d0f1a', border: 'none', borderRadius: '10px', padding: '14px 32px', fontSize: '15px', fontWeight: '800', cursor: 'pointer', fontFamily: 'inherit', width: '100%' } }
...
export default function Page() {
  ...
  const handlePay = async () => {
    ...
    try {
      const res = await fetch('/api/checkout-freelance-contract-template', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
  ...
  const handleGenerate = async () => {
    setLoading(true); setError('')
    try {
      const res = await fetch('/api/generate-freelance-contract-template', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
      const data = await res.json()
  ...
  return (
    <>
      <Head><title>Freelance Contract Template — CareerCraft AI</title><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;600;700;800&display=swap" rel="stylesheet" /></Head>
      <div style={S.page}>
        <nav style={S.nav}>
        ...
        <div style={S.wrap}>
          <div style={{ marginBottom: '28px' }}>
            <div style={{ fontSize: '11px', color: '#34d399', fontWeight: '700', letterSpacing: '0.1em', marginBottom: '8px' }}>FREELANCECRAFT AI</div>
            <h1 style={{ fontSize: '32px', fontWeight: '800', letterSpacing: '-0.03em', marginBottom: '8px' }}>Freelance Contract Template</h1>
            <p style={{ color: '#8a8070', fontSize: '15px' }}>Protect yourself and impress clients. $16.99 one-time.</p>
          </div>
          ...
          {!paid ? (
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '24px', textAlign: 'center' }}>
              <div style={{ fontSize: '32px', fontWeight: '900', color: '#34d399', marginBottom: '4px' }}>$16.99</div>
