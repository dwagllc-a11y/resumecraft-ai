...
export default function CoverLetter() {
  ...
  const [form, setForm] = useState({ name: '', role: '', company: '', experience: '', whyCompany: '', tone: 'professional' })
  ...
  useState(() => {
    ...
    if (session_id && p === 'true') {
      ...
      fetch('/api/verify-payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ session_id }) })
        .then(r => r.json()).then(d => { if (d.paid) setPaid(true); setVerifying(false) }).catch(() => setVerifying(false))
    }
  }, [router.query])
  ...
  const handlePay = async () => {
    ...
    try {
      const res = await fetch('/api/checkout-cover-letter', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
  ...
  const handleGenerate = async () => {
    setLoading(true); setError('')
    try {
      const res = await fetch('/api/generate-cover-letter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
      const data = await res.json()
  ...
  return (
    <>
      <Head><title>Cover Letter Generator — CareerCraft AI</title><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;600;700;800&display=swap" rel="stylesheet" /></Head>
      <div style={S.page}>
        <nav style={S.nav}>
          ...
          {verifying ? <div style={{ color: '#8a8070' }}>Verifying payment...</div> : (
            <>
              <div style={{ display: 'grid', gap: '16px', marginBottom: '24px' }}>
                {[['Your Full Name', 'name', 'text'], ['Target Role', 'role', 'text'], ['Company Name', 'company', 'text']].map(([label, key]) => (
                  <div key={key}><label style={S.label}>{label}</label><input style={S.input} value={form[key]} onChange={set(key)} placeholder={label} /></div>
                ))}
                <div><label style={S.label}>Your Relevant Experience (2-3 sentences)</label><textarea style={{ ...S.input, height: '80px', resize: 'vertical' }} value={form.experience} onChange={set('experience')} placeholder="e.g. 5 years in B2B SaaS sales, exceeded quota 4 years running..." /></div>
                <div><label style={S.label}>Why This Company? (1-2 sentences)</label><textarea style={{ ...S.input, height: '70px', resize: 'vertical' }} value={form.whyCompany} onChange={set('whyCompany')} placeholder="e.g. Admire their mission to democratize AI tools..." /></div>
                <div><label style={S.label}>Tone</label>
                  <select style={S.input} value={form.tone} onChange={set('tone')}>
                    {['professional', 'enthusiastic', 'confident', 'conversational'].map(t => <option key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}</option>)}
                  </select>
                </div>
              </div>
              ...
              {!paid ? (
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <button style={S.btn} onClick={handleGenerate} disabled={loading}>{loading ? 'Writing your letter...' : '✦ Generate Cover Letter'}</button>
                  {error && <div style={{ color: '#ef4444', fontSize: '13px', marginTop: '10px' }}>{error}</div>}
                </div>
              ) : (
                <div>
