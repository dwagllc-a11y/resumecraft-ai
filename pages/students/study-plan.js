...
const FIELDS = ['name', 'major', 'year', 'targetCareer', 'currentGPA', 'strengths', 'weaknesses', 'timeline']
...
export default function Page() {
  ...
  const handlePay = async () => {
    ...
    try {
      const res = await fetch('/api/checkout-student-study-plan', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
  ...
  const handleGenerate = async () => {
    setLoading(true); setError('')
    try {
      const res = await fetch('/api/generate-student-study-plan', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
      const data = await res.json()
  ...
  return (
    <>
      <Head><title>Study & Career Plan — CareerCraft AI</title><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;600;700;800&display=swap" rel="stylesheet" /></Head>
      <div style={S.page}>
        <nav style={S.nav}>
        ...
        <div style={S.wrap}>
          <div style={{ marginBottom: '28px' }}>
            <div style={{ fontSize: '11px', color: '#f472b6', fontWeight: '700', letterSpacing: '0.1em', marginBottom: '8px' }}>STUDENTCRAFT AI</div>
            <h1 style={{ fontSize: '32px', fontWeight: '800', letterSpacing: '-0.03em', marginBottom: '8px' }}>Study & Career Plan</h1>
            <p style={{ color: '#8a8070', fontSize: '15px' }}>Your roadmap from now to dream job. $16.99 one-time.</p>
          </div>
