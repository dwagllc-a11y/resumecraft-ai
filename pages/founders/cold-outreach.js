...
const FIELDS = ['founderName', 'startupName', 'targetType', 'purpose', 'background']
...
export default function Page() {
  ...
  const handlePay = async () => {
    ...
    try {
      const res = await fetch('/api/checkout-founder-cold-outreach', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
  ...
  const handleGenerate = async () => {
    setLoading(true); setError('')
    try {
      const res = await fetch('/api/generate-founder-cold-outreach', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
      const data = await res.json()
  ...
  return (
    <>
      <Head><title>Founder Cold Outreach Scripts — CareerCraft AI</title><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;600;700;800&display=swap" rel="stylesheet" /></Head>
      <div style={S.page}>
        <nav style={S.nav}>
        ...
        <div style={S.wrap}>
          <div style={{ marginBottom: '28px' }}>
            <div style={{ fontSize: '11px', color: '#34d399', fontWeight: '700', letterSpacing: '0.1em', marginBottom: '8px' }}>FOUNDERCRAFT AI</div>
            <h1 style={{ fontSize: '32px', fontWeight: '800', letterSpacing: '-0.03em', marginBottom: '8px' }}>Founder Cold Outreach Scripts</h1>
            <p style={{ color: '#8a8070', fontSize: '15px' }}>Reach advisors, press, and partners. $16.99 one-time.</p>
          </div>
